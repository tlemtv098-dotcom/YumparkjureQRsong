# Search Fast Fallback Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** ค้นหาไม่ค้าง loading — abort request เก่า + timeout 8 วิ + API ก่อน fallback ทันที

**Estimated tasks:** 4 | **Estimated time:** ~40 min | **Touches:** API / Frontend / Tests

## Current Problem / Current Solution

- พิมพ์ค้นหาแล้วค้างที่ `AI กำลังช่วยค้นเพลง...` ทั้ง player (`#manual-loading`) และ request (`#loading`)
- `search_youtube` ลอง API (8s) → yt-dlp fallback (ช้า, Render 429/bot) → `_is_embeddable` ทีละวิดีโอ (3s/ตัว) รวมเกิน 30 วิ
- Frontend `fetch("/api/search/")` ไม่มี AbortController, ไม่มี timeout, `player` ไม่เช็ค `res.ok` → request เก่าค้างซ้อนกัน

## Proposed Approach

- Backend: `search_song` ใช้ API-only เร็ว — ถ้า `youtube_api_search` ว่างให้คืน fallback static ทันที ไม่เรียก yt-dlp/`_is_embeddable` ใน path ค้นหา (เก็บ deep check ไว้แค่ `hits` ถ้าจำเป็น)
- Frontend ทั้งสองหน้า: AbortController ยกเลิก request เก่าเมื่อพิมพ์ใหม่ + timeout 8 วิ + เช็ค `res.ok` + ซ่อน loading ทุก path
...
| -------- | ------ | ----- |
| พิมพ์ "ข้างกัน" รัวๆ | request เก่าค้างซ้อน ค้าง loading | abort ตัวเก่า เหลือตัวล่าสุด + timeout 8 วิ |
| API key หมด/quota | รอ yt-dlp 10-30 วิแล้วว่าง | คืน fallback 3 เพลงทันที (<1 วิ) |
| /api/search 500/timeout | loading ค้างตลอด | toast + ซ่อน loading + โชว์ fallback |

## Assumptions & Risks

- **Assumed:** `YOUTUBE_API_KEY` มีบน Render (เคยใส่แล้ว) — API path เร็วพอ
- **Assumed:** ยอมรับ fallback 3 เพลงเมื่อ API ว่าง (ตาม Q3)
- **Risk:** ถ้า API key หมด จะได้แค่ fallback 3 เพลงซ้ำๆ — รับได้ดีกว่าค้าง
- **Risk:** ตัด `_is_embeddable` ออกจาก search อาจหลุดเพลง 153 มาบ้าง — player soft-skip จัดการต่อ

## Impact

- ค้นหาไม่ค้างทั้งสองหน้า
...
1. **[Backend search fast]** - Lane A | Can run together: Task 2, Task 3 | Must wait for: none | TDD slice: slow API returns fallback <1s -> remove yt-dlp/embed from search path -> `manage.py test`
...
- `search_song`: when `results` empty, return filtered static fallback (5 entries) immediately as today, but ensure the fallback filter (`_is_blocked`, `_is_album_title`, `_is_ai_title`, `_is_non_music`) cannot empty it silently — if filtered empty, return `fallback[:3]` unfiltered by album (keep blocked filter).
...
- In `manualSearch()`: `if (manualSearchAbort) manualSearchAbort.abort(); manualSearchAbort = new AbortController();` pass `signal` to `fetch`, add `setTimeout` 8000 abort.
...
- Keep `blockedVideoIds`, `isAlbumLike`, `isAI`, `escapeHtml`, debounce 500ms untouched.
...
def test_request_search_has_abort():
    ...
    html = res.content.decode()
    assert "AbortController" in html
    assert "กำลังค้นหาโปรดรอสักครู่" in html or "AI กำลังช่วยค้นเพลง" in html
```
...
Run `venv\Scripts\python.exe manage.py test music.tests -v2` — all 50+ PASS, plus `venv\Scripts\python.exe manage.py check` PASS.
